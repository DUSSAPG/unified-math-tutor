...
void main() {
  ...
  group('First-use guided walkthrough', () {
    testWidgets(
        'shows once for a fresh profile, contains the mission steps, and dismisses',
        (tester) async {
      expect(
        InteractiveLabsProgressService.instance
            .hasSeenFirstUse(InteractiveLabId.flightPathLab),
        isFalse,
      );
      ...
      expect(find.text('Point the plane toward the yellow target.'),
          findsOneWidget);
      expect(
          find.text('Choose how far the plane should travel.'), findsOneWidget);
      expect(find.text('Press Test Flight to see where it lands.'),
          findsOneWidget);
      ...
      expect(
        ...
        isTrue,
        reason:
            'Dismissing the walkthrough must persist so it does not show again',
      );
      expect(tester.takeException(), isNull);
    });

    testWidgets('does not reappear once already seen for this profile',
        (tester) async {
  ...
  group('Explorer/Builder/Navigator presentation', () {
    ...
    testWidgets(
        'Builder (default) level shows a compass direction alongside the formal bearing',
        (tester) async {
      await pump(tester);
      expect(
    ...
    testWidgets(
        'Navigator level also shows the compass direction alongside the formal bearing',
        (tester) async {
      await InteractiveLabsProgressService.instance
          .setGuidanceLevel(LabGuidanceLevel.navigator);
      await pump(tester);
    ...
    testWidgets(
        'Explorer level still shows both the direction word and the degree value',
        (tester) async {
      await InteractiveLabsProgressService.instance
          .setGuidanceLevel(LabGuidanceLevel.explorer);
      await pump(tester);
  ...
  group('Guidance level persistence', () {
    testWidgets(
        'chosen level persists per learner profile and does not leak to another',
        (tester) async {
      final learnerAId =
      ...
      await InteractiveLabsProgressService.instance
          .setGuidanceLevel(LabGuidanceLevel.navigator);
      expect(InteractiveLabsProgressService.instance.guidanceLevel(),
  ...
  group('Explanation-seen tracking', () {
    test('defaults to unseen, then persists true once marked', () async {
      expect(
        InteractiveLabsProgressService.instance
    ...
    test('is tracked independently per lab', () async {
      await InteractiveLabsProgressService.instance
          .markExplanationSeen(InteractiveLabId.footballPrecision);
      ...
      expect(
        ...
        isFalse,
        reason: 'Marking one lab seen must not leak to a different lab',
      );
    });

    testWidgets(
        'chosen state persists per learner profile and does not leak to another',
        (tester) async {
      final learnerAId =
          await LearnerProfilesService.instance.addLearner('Learner A');
      final learnerBId =
      ...
      expect(
        InteractiveLabsProgressService.instance
            .hasSeenExplanation(InteractiveLabId.mazeDriver),
        isFalse,
        reason: 'Learner B must not inherit Learner A\'s seen-explanation flag',
      );
  ...
  group('Responsive layout', () {
    testWidgets(
        'renders without a RenderFlex overflow on a small phone height (390x600)',
        (tester) async {
      await InteractiveLabsProgressService.instance
          .markFirstUseSeen(InteractiveLabId.flightPathLab);
    ...
    testWidgets(
        'renders without overflow in landscape on a small phone (844x390)',
        (tester) async {
      await InteractiveLabsProgressService.instance
          .markFirstUseSeen(InteractiveLabId.flightPathLab);
  ...
  group('Accessibility', () {
    testWidgets('Help and Reset controls expose semantic labels',
        (tester) async {
      await InteractiveLabsProgressService.instance
          .markFirstUseSeen(InteractiveLabId.flightPathLab);
      await pump(tester);
    ...
    testWidgets(
        'opening Help exposes all four required sections and the guidance selector',
        (tester) async {
      await InteractiveLabsProgressService.instance
          .markFirstUseSeen(InteractiveLabId.flightPathLab);
      await pump(tester);
    ...
    testWidgets(
        'changing guidance level from within Help updates the lab immediately',
        (tester) async {
      await InteractiveLabsProgressService.instance
          .markFirstUseSeen(InteractiveLabId.flightPathLab);
      await pump(tester);
      ...
      final navigatorChip = tester
          .widget<ChoiceChip>(find.widgetWithText(ChoiceChip, 'Navigator'));
