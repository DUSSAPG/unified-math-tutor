...
void main() {
  ...
  test('throws when a visual asset id is set without a matching frontVisualAlt',
      () async {
  ...
  test('byTopic and byType filter correctly against the real bundled catalog',
      () async {
    final numberCards =
        await RecallCardCatalogService.instance.byTopic(RecallTopic.number);
    expect(numberCards, hasLength(20));
    expect(numberCards.every((c) => c.topicId == RecallTopic.number), isTrue);

    final formulaCards =
        await RecallCardCatalogService.instance.byType(RecallCardType.formula);
    expect(formulaCards, isNotEmpty);
    expect(formulaCards.every((c) => c.cardType == RecallCardType.formula),
        isTrue);
  });

  test('search matches on id, topic, type and English prompt/answer text',
      () async {
    final results =
        await RecallCardCatalogService.instance.search('pythagoras');
