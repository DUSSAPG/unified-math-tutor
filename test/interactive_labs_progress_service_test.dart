...
void main() {
  ...
  test('a lab with no activity starts at zero attempts and zero completions', () {
    expect(InteractiveLabsProgressService.instance.attemptsFor(InteractiveLabId.fractionBuilder), 0);
    expect(InteractiveLabsProgressService.instance.completedFor(InteractiveLabId.fractionBuilder), 0);
  });

  test('recordAttempt increments attempts without affecting completions', () async {
    await InteractiveLabsProgressService.instance.recordAttempt(InteractiveLabId.algebraBalance);
    await InteractiveLabsProgressService.instance.recordAttempt(InteractiveLabId.algebraBalance);
    expect(InteractiveLabsProgressService.instance.attemptsFor(InteractiveLabId.algebraBalance), 2);
    expect(InteractiveLabsProgressService.instance.completedFor(InteractiveLabId.algebraBalance), 0);
  });

  test('unlimited retries: repeated completions all count (no cap, no scoring)', () async {
    for (var i = 0; i < 5; i++) {
      await InteractiveLabsProgressService.instance.recordCompletion(InteractiveLabId.dataDetective);
    }
    expect(InteractiveLabsProgressService.instance.completedFor(InteractiveLabId.dataDetective), 5);
  });
  ...
  test('attempts/completions are tracked independently per lab', () async {
    ...
    expect(InteractiveLabsProgressService.instance.attemptsFor(InteractiveLabId.numberLineExplorer), 0);
    expect(InteractiveLabsProgressService.instance.completedFor(InteractiveLabId.numberLineExplorer), 0);
  });

  test('linked Recall/Discovery/Practice use counters increment independently', () async {
    ...
    expect(InteractiveLabsProgressService.instance.linkedRecallUseCountFor(lab), 2);
    expect(InteractiveLabsProgressService.instance.linkedDiscoveryUseCountFor(lab), 1);
    expect(InteractiveLabsProgressService.instance.linkedPracticeUseCountFor(lab), 1);
  });
  ...
  test('progress does not leak between learner profiles', () async {
    ...
    await InteractiveLabsProgressService.instance.recordCompletion(InteractiveLabId.fractionBuilder);
    expect(InteractiveLabsProgressService.instance.completedFor(InteractiveLabId.fractionBuilder), 1);
